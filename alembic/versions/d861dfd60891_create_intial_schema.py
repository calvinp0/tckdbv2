...
def upgrade() -> None:
    ...
    op.execute("""
               CREATE TYPE a_units AS ENUM ('cm3_mol_s',
                                            'cm3_molecule_s',
                                            's_1');
...
CREATE TYPE kinetics_model_kind AS ENUM (
  ...
  'fit_source'
    ...
    op.execute("""
        CREATE TYPE tunneling_model_kind AS ENUM (
  'none',
  'wigner',
  'eckart',
  'skodje_truhlar'
...
CREATE TABLE "kinetics" (
  ...
  "t0_k" DOUBLE PRECISION,
  "tmin_k" DOUBLE PRECISION,
  "tmax_k" DOUBLE PRECISION,
  "degeneracy" DOUBLE PRECISION,
  "tunneling_model" tunneling_model_kind,
  "a_uncertainty" DOUBLE PRECISION,
  "n_uncertainty" DOUBLE PRECISION,
  "ea_uncertainty_kj_mol" DOUBLE PRECISION,
  "note" text
  ,
  CONSTRAINT "ck_kinetics_kinetics_t0_k_gt_0" CHECK ("t0_k" IS NULL OR "t0_k" > 0),
  CONSTRAINT "ck_kinetics_kinetics_tmin_k_gt_0" CHECK ("tmin_k" IS NULL OR "tmin_k" > 0),
  CONSTRAINT "ck_kinetics_kinetics_tmax_k_gt_0" CHECK ("tmax_k" IS NULL OR "tmax_k" > 0),
  CONSTRAINT "ck_kinetics_kinetics_tmin_le_tmax" CHECK ("tmin_k" IS NULL OR "tmax_k" IS NULL OR "tmin_k" <= "tmax_k"),
  CONSTRAINT "ck_kinetics_kinetics_degeneracy_gt_0" CHECK ("degeneracy" IS NULL OR "degeneracy" > 0)
);""")
    op.execute("""
...
CREATE TABLE "network" (
  ...
  "name" text,
  "description" text,
  "is_pressure_dependent" BOOLEAN NOT NULL DEFAULT true,
  "method" text,
  "tmin_k" double precision,
  "tmax_k" double precision,
  "pmin_bar" double precision,
  "pmax_bar" double precision,
  "maximum_grain_size_kj_mol" double precision,
  "minimum_grain_count" integer,
  "literature_id" bigint,
  "software_release_id" bigint,
  "workflow_tool_release_id" bigint,
  "created_by" bigint,
...
CREATE TABLE "workflow_tool" (
  ...
  "created_at" timestamp NOT NULL DEFAULT now()
    ...
    op.execute("""
CREATE TABLE "workflow_tool_release" (
  "id" bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
...
CREATE UNIQUE INDEX "geometry_geom_hash_uq" ON "geometry" USING btree ("geom_hash");""")
    op.execute("""
CREATE UNIQUE INDEX "software_name_uq" ON "software" ("name");""")
    op.execute("""
CREATE UNIQUE INDEX "software_release_dedupe_uq"
ON "software_release" (
  "software_id",
  COALESCE("version", ''),
  COALESCE("revision", ''),
...
CREATE UNIQUE INDEX "literature_author_unique_uq" ON "literature_author" ("literature_id", "author_id");"""
    ...
    op.execute("""
CREATE UNIQUE INDEX "workflow_tool_name_uq" ON "workflow_tool" ("name");""")
    op.execute("""
CREATE UNIQUE INDEX "workflow_tool_release_dedupe_uq"
ON "workflow_tool_release" (
  "workflow_tool_id",
  COALESCE("version", ''),
  COALESCE("git_commit", ''),
  COALESCE("git_branch", ''),
...
COMMENT ON COLUMN "geometry_atom"."atom_index" IS '1..natoms';""")
    op.execute("""
COMMENT ON COLUMN "software_release"."build" IS 'optional';""")
    op.execute("""
...
ALTER TABLE "transport" ADD FOREIGN KEY ("literature_id") REFERENCES "literature" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    ...
    op.execute(
        """
ALTER TABLE "transport" ADD FOREIGN KEY ("software_id") REFERENCES "software_release" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    )
    op.execute(
        """
ALTER TABLE "transport" ADD FOREIGN KEY ("workflow_tool_id") REFERENCES "workflow_tool_release" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    )
    op.execute(
        """
...
ALTER TABLE "reaction_entry" ADD FOREIGN KEY ("preferred_ts_entry_id") REFERENCES "transition_state_entry" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    ...
    op.execute(
        """
ALTER TABLE "reaction_entry" ADD FOREIGN KEY ("preferred_kinetics_id") REFERENCES "kinetics" ("id") DEFERRABLE INITIALLY DEFERRED;"""
    )
    op.execute(
        """
...
ALTER TABLE "kinetics" ADD FOREIGN KEY ("literature_id") REFERENCES "literature" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    ...
    op.execute(
        """
ALTER TABLE "kinetics" ADD FOREIGN KEY ("workflow_tool_release_id") REFERENCES "workflow_tool_release" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    )
    op.execute(
        """
ALTER TABLE "kinetics" ADD FOREIGN KEY ("software_release_id") REFERENCES "software_release" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    )
    op.execute(
        """
...
ALTER TABLE "network" ADD FOREIGN KEY ("literature_id") REFERENCES "literature" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    ...
    op.execute(
        """
ALTER TABLE "network" ADD FOREIGN KEY ("software_release_id") REFERENCES "software_release" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    )
    op.execute(
        """
ALTER TABLE "software_release" ADD FOREIGN KEY ("software_id") REFERENCES "software" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    )
    op.execute(
        """
ALTER TABLE "network" ADD FOREIGN KEY ("workflow_tool_release_id") REFERENCES "workflow_tool_release" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    )
    op.execute(
        """
ALTER TABLE "workflow_tool_release" ADD FOREIGN KEY ("workflow_tool_id") REFERENCES "workflow_tool" ("id") DEFERRABLE INITIALLY IMMEDIATE;"""
    )
    op.execute(
        """
...
def downgrade() -> None:
    ...
    op.execute(
        'ALTER TABLE "reaction_entry" DROP CONSTRAINT IF EXISTS "reaction_entry_preferred_kinetics_id_fkey";'
    )
    op.execute(
    ...
    op.execute('DROP TABLE IF EXISTS "workflow_tool_release";')
    op.execute('DROP TABLE IF EXISTS "workflow_tool";')
    op.execute('DROP TABLE IF EXISTS "software_release";')
    ...
    op.execute("DROP TYPE IF EXISTS tunneling_model_kind;")
    ...
    op.execute("DROP TYPE IF EXISTS a_units;")
