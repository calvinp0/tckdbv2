...
from sqlalchemy import CHAR, Enum as SAEnum, ForeignKey, Integer, SmallInteger, Text, func
...
class Species(Base, TimestampMixin):
    ...
    kind: Mapped[MoleculeKind] = mapped_column(SAEnum(MoleculeKind, name="molecule_kind"), nullable=False)
...
class SpeciesEntry(Base, TimestampMixin):
    ...
    kind: Mapped[StationaryPointKind] = mapped_column(SAEnum(StationaryPointKind, name="stationary_point_kind"),
                                                      nullable=False)
    ...
    preferred_calculation: Mapped[Optional["Calculation"]] = relationship(
        foreign_keys=[preferred_calculation_id],
        post_update=True
    )
