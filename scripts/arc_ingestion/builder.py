...
def _make_opt_calculation(
    ...
    # Attach the SP log as an artifact (for opt, the SP log IS the opt output)
    artifacts = []
...
def _build_species_payload(
    ...
    conformer = {
        ...
        "calculation": _make_opt_calculation(opt_key, None, run, sp_info),
    }
    ...
    if sp_info.freq_result:
        ...
        calculations.append(
            _make_freq_calculation(freq_key, geom_key, run, sp_info)
        )
    ...
    if not run.sp_is_same_as_opt:
        sp_key = f"{label}_sp"
        calculations.append(
            _make_sp_calculation(sp_key, geom_key, run, sp_info)
        )
...
def _build_ts_payload(
    ...
    primary_calc = _make_opt_calculation(opt_key, None, run, ts_info)

    calculations = []
    if ts_info.freq_result:
        freq_key = f"{ts_info.label}_freq"
        calculations.append(
            _make_freq_calculation(freq_key, geom_key, run, ts_info)
        )
...
def build_payload(run: ARCRunData, arc_dir: Path) -> dict:
    ...
    for label in rxn.reactant_labels + rxn.product_labels:
        ...
        species_payloads.append(_build_species_payload(label, sp_info, run))

    # Build TS payload
    ts_payload = None
    ts_info = run.transition_states.get(rxn.ts_label)
    if ts_info and ts_info.converged:
        ts_payload = _build_ts_payload(ts_info, run)

    # Parse kinetics from Arkane output
    ...
    if arkane_output.exists():
        ...
        raise ValueError(f"Arkane output not found at {arkane_output}")
